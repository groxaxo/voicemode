...
class TestSendCommand:
    ...
    def test_send_delivers_message_when_running(self, runner):
        ...
        # Verify send-keys was called with -l for literal to pane 0
        calls = mock_run.call_args_list
        assert len(calls) == 2  # Message + Enter
        assert calls[0][0][0] == ['tmux', 'send-keys', '-t', 'voicemode:operator.0', '-l', 'Hello, world!']
        assert calls[1][0][0] == ['tmux', 'send-keys', '-t', 'voicemode:operator.0', 'Enter']
    ...
    def test_send_uses_custom_session_name(self, runner):
        ...
        calls = mock_run.call_args_list
        assert calls[0][0][0][3] == 'custom:operator.0'
    ...
    def test_send_truncates_long_messages_in_output(self, runner):
        ...
        # But the full message should be sent via send-keys -l
        calls = mock_run.call_args_list
    ...
    def test_send_waits_after_auto_start(self, runner, temp_home):
        """Should auto-start agent when not running and pass message as initial prompt."""
        mock_run = MagicMock(return_value=MagicMock(returncode=0))
        ...
        assert result.exit_code == 0
        # The auto-start should have invoked the start command which sends the message
        # as initial prompt to claude via build_claude_command
        assert "Starting agent" in result.output
        assert "Started with:" in result.output
